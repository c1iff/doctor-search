...
DoctorSearch.prototype.getDoctorsByName = function(searchName, specialtieSearch, displayFunction) {
  var searchString = 'name=' + string.replace();
  var searchNameArray = name.split('').map(function(x) {
    if (x === ' ') {
      x = '%20'
    }
    console.log(x);
  ...
  searchString = 'name=' + searchNameArray.join()
  console.log(searchString);
  $.get('https://api.betterdoctor.com/2016-03-01/doctors?name='+ searchString + '&specialty_uid=pediatrician&location=45.5231%2C-122.6765%2C%205&user_location=45.5231%2C-122.6765&skip=0&limit=20&user_key=' + apiKey)
    .then(function(results) {
